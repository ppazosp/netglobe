class Markers {
  constructor(countries, {
    ...
    this.countries = countries.filter(country => {
      const isKey = Object.keys(data.connections).includes(country.name);
      const isValue = Object.values(data.connections).some(connectedCountries =>
        connectedCountries.some(c => c.name === country.name)
      );
